...
const pay = () => {
    ...
    return (
        <Flex flexDirection={"column"} w={"50%"} h="100vh" p={"2%"} ml={"25%"}>
            ...
            <Text fontWeight={"bold"} fontSize="3xl" alignSelf={"center"} mb="7%">お客様情報の入力</Text>
            ...
            <Flex justifyContent={"space-between"} alignItems={"center"}>
                ...
                <Flex w={"48%"} mr="20%" alignContent={"center"} alignItems="center" justifyContent={"space-around"}>
                    ...
                    <Input required w={"20%"} />
                    ?
                    <Button onClick={() => window.open("https://www.post.japanpost.jp/zipcode/", "_blank")}>
                        郵便番号検索
                    </Button>
                </Flex>
            </Flex>
            ...
            <Flex justifyContent={"space-between"} alignItems={"center"}>
                ...
                <RadioGroup w={"68%"} onChange={setRadio} value={radio}>
                    <Radio required value='1'>配信</Radio>
                    <Radio required value='2' ml={"10px"}>解除</Radio>
                </RadioGroup>
            </Flex>

            <Flex justifyContent={"space-between"} h="100px" mt={"10%"}>
                <Button w="23%" fontSize={"2xl"} onClick={() => router.back()}>← 戻る</Button>
                <Link href={`/concerts/${router.query.id}/products/check`}>
                    <Button fontSize={"2xl"} w="23%">次へ →</Button>
                </Link>
            </Flex>
        </Flex>
    )
