...
const ModelMotion: FC<{ mediaStream: MediaStream }> = ({ mediaStream }) => {
  ...
  useEffect(() => {
    console.log("peers변경됨", peers.length, peers);
  }, [peers]);
  ...
  const onResults = useCallback(
    (results: Results) => {
      if (
        ...
        console.log("여긴 테스트", peers.length);
        ...
        if (peers) {
          // console.log("onResults peers", peers);
          // console.log("onResults global", globalPeer);
          // console.log("onResults model", model);
          // console.log("onResults motion", motion);
